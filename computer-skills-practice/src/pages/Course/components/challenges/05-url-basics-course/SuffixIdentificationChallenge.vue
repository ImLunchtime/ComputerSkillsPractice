  ...
  <div class="challenge-container">
    ...
    <div v-if="!completed" class="mb-8">
      ...
      <div class="url-display bg-gray-100 border-2 border-gray-300 rounded-lg p-6 max-w-3xl mx-auto mb-6">
        <div class="text-center">
          ...
          <div class="text-2xl font-mono bg-white border border-gray-200 rounded px-4 py-3 inline-block">
            ...
            <span class="text-green-600">{{ currentUrl.domain }}</span>
            <span class="text-gray-600">.</span>
            <span 
              class="suffix-part cursor-pointer hover:bg-purple-200 px-1 rounded transition-colors"
              @click="handleSuffixClick"
              :class="{ 'bg-purple-300': isClicked }"
            >{{ currentUrl.suffix }}</span>
      ...
      <div class="mt-6 text-sm text-gray-500">
        ...
        提示：域名后缀（顶级域名）位于一级域名之后，如.com、.org、.net等
      </div>
    </div>
    ...
    <div v-if="showStructure && !completed" class="structure-explanation bg-blue-50 border border-blue-200 rounded-lg p-6 max-w-3xl mx-auto mb-6">
      ...
      <div class="text-center mb-4">
        <div class="text-xl font-mono bg-white border border-blue-200 rounded px-4 py-3 inline-block">
          ...
          <span>.</span>
          <span class="bg-yellow-200 px-1 rounded">{{ currentUrl.domain }}</span>
          <span>.</span>
          <span class="bg-purple-200 px-1 rounded font-bold">{{ currentUrl.suffix }}</span>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        ...
        <div class="text-center">
          <div class="bg-yellow-200 px-2 py-1 rounded mb-1">一级域名</div>
          <div class="text-gray-600">{{ currentUrl.domain }}</div>
        </div>
        <div class="text-center">
          <div class="bg-purple-200 px-2 py-1 rounded mb-1 font-bold">域名后缀</div>
    ...
    <div v-else-if="completed" class="completion-animation">
      ...
      <p class="text-gray-600 mb-4">很好！你已经学会了识别URL中的域名后缀</p>
      ...
      <div class="bg-green-50 border border-green-200 rounded-lg p-6 max-w-3xl mx-auto">
        ...
        <div class="text-sm text-green-800 mb-2">
          <strong>域名后缀：</strong>{{ currentUrl.suffix }} - 这是顶级域名，表示网站的类型或地区
        </div>
        <div class="text-xs text-green-700">
          <strong>常见后缀：</strong>.com（商业）、.org（组织）、.net（网络）、.edu（教育）、.gov（政府）
        </div>
      </div>
    </div>
  </div>
...
// URL示例数据
const urlExamples = [
  { domain: 'amazon', suffix: 'com', path: '/products/electronics' },
  { domain: 'wikipedia', suffix: 'org', path: '/wiki/Technology' },
  { domain: 'cloudflare', suffix: 'net', path: '/dns' },
  { domain: 'stanford', suffix: 'edu', path: '/academics' },
  { domain: 'nasa', suffix: 'gov', path: '/missions' },
  { domain: 'reddit', suffix: 'com', path: '/r/programming' }
]
